...
impl Ui {
    pub fn build(application: &gtk::Application) -> Result<Self> {
        ...
        disks_listbox.bind_model(Some(&disks_model), move |item| {
            ...
            //hbox.paappendck_start(&size_label);
            // TODO
            //row.add(&hbox);
            //row.show_all();
        ...
        let install_progress: gtk::ProgressBar =
            install_builder.get_progress_bar("install_progress")?;
        ...
        receiver.attach(None,glib::clone!(@strong ui, @strong application =>  move |msg| {
            match msg {
                Msg::InstallStarted => {
                    ...
                    let buffer = ui.install_textview.buffer();
    ...
    fn get_disks() -> Result<Vec<RowData>> {
        ...
        let proxy = conn.with_proxy(
            "com.subgraph.installer",
            "/com/subgraph/installer",
            Duration::from_millis(5000),
        );
        let (devices,): (HashMap<String, Vec<String>>,) = proxy
            .method_call("com.subgraph.installer.Manager", "GetDisks", ())
            .map_err(Error::Dbus)?;
